...
resource "aws_opensearchserverless_security_policy" "encryption" {
  name        = "cg-kb-enc-${random_id.suffix.hex}"
  type        = "encryption"
  description = "Encryption policy for CloudGoat KB collection"

  policy = jsonencode({
    Rules = [
      {
        ResourceType = "collection"
        Resource     = ["collection/cg-kb-${random_id.suffix.hex}"]
      }
    ]
    AWSOwnedKey = true
  })
...
resource "aws_opensearchserverless_security_policy" "network" {
  name        = "cg-kb-net-${random_id.suffix.hex}"
  type        = "network"
  description = "Network policy for CloudGoat KB collection"
...
resource "aws_opensearchserverless_access_policy" "kb_access" {
  name        = "cg-kb-access-${random_id.suffix.hex}"
  type        = "data"
  description = "Data access policy for CloudGoat KB"
  ...
  policy = jsonencode([
    {
      Rules = [
        {
          ResourceType = "index"
          Resource     = ["index/cg-kb-${random_id.suffix.hex}/*"]
          Permission   = ["aoss:*"]
        },
        {
          ResourceType = "collection"
...
resource "aws_opensearchserverless_collection" "kb" {
  name        = "cg-kb-${random_id.suffix.hex}"
  type        = "VECTORSEARCH"
  description = "Vector store for CloudGoat AI RAG scenario"
...
resource "aws_iam_role_policy" "bedrock_opensearch" {
  name = "cg-bedrock-opensearch-${var.cgid}"
...
resource "aws_bedrockagent_knowledge_base" "main" {
  name        = "cg-knowledge-base-${var.cgid}"
...
resource "aws_bedrockagent_agent" "assistant" {
  agent_name                  = "cg-acme-assistant-${var.cgid}"
